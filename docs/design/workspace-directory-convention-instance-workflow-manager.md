...
> **范围收窄追注（FIX-72，2026-09-16，人工裁定）**：本决策的**范围收窄为「归档位置统一到 `docs/tasks/archive/`」**。原表述中「删除 `specs/`」一项自 2026-09-16 起失效——编号迁移（d34f618）确立「定义入库」新规，`docs/tasks/specs/<TASK_ID>-<slug>/` **复位为定义产物的正式家**（规格 + Definition Check + 分析摘要 + 决策票），不再是废弃目录；工作区闸门 D-8 相应不再对 specs/ 报废弃警告，改为检测 archive/ 内混入的非任务目录（同名漂移）。本追注不改写原决策正文（R-7）。
...
| 项 | 内容 | 阻塞 |
|---|---|---|
| ~~B4~~ | ✅ **已完成（2026-09-13）**：19 个运行时工作区 + 2 个死目录（cursor/hplt、cursor/zn9w）全部回收，19 个 `vwf/run/*` 等占用分支删除，`worktree list` 仅剩主检出。**过程发现**：本机 safe-delete 钩子使 `git worktree remove` 报假错（"not a working tree"）但实际生效——执行后必须以 `worktree list` 复核为准；磁盘目录残留用 mv 入回收区清理 | — |
...
| 例外区治理 | 运行时 `ws-*/source` 目录名不等于分支名 | ✅ 随 B4 一并消除（运行时工作区已全部回收，不再存在例外区条目）；未来新开的运行时工作区仍按 §1.10 属适配段责任 |
| DSH 产品侧长效机制 | workspace registry 增加终态 GC（会话结束 + N 天自动回收）——本次 19 个残留即其缺失的直接后果 | 属 DSH 产品工作项 |
