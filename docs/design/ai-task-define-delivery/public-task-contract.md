...
> **产品拍板（相对上游规格的覆盖）**：自动返工上限 **3**（非上游文中的 2）；人工验收严格三态；定义能力升级现有「需求分析」，不新建第二套入口。  
> **本文件职责**：定义 / 单任务交付 / Execution Plan 三块能力共用的任务字段、状态与版本规则。不单独建设第四套系统。  
> **M1 范围**：契约 + 定义落档物齐备；交付入口完整改造属 M2。
...
- 有前置依赖任务的自动接续（V0.2）
- 把历史建设 handoff schema 的 `user_accepted` 立刻改名为 `CONDITIONAL_PASS`（留待 M2，避免本里程碑扩大范围）
