...
> **版本**：**v0.1.9 + M2 overlay（2026-09-05）** —— 正文七阶段证据底物以 v0.1.9 冻结为准（v0.1.9 为**纯放宽**：验收包存在性分层，`record_version` 仍钉 `v0.1.8`，历史记录全部继续合法）；**产品可见单任务交付主链以** `docs/design/ai-task-define-delivery/single-task-delivery-m2.md` **为准（定义外置）**。  
