...
> 说明（不进机器解析字段）：
> - 收口后状态：2026-09-21 人工验收通过并经 PR #255 合并（merge `f0d9520`），归档存根见 `docs/tasks/archive/CHORE-110/`。
> - 原 `当前状态 = 待确认` 的依据：Definition Check 已全过、未决产品事项 0（DT-01 两项开放项于 2026-09-19 裁定：A·b3 存在性分层 + 摘要修复并入本票）；待人工一句「确认需求基线 V1，可进入交付」即可转「已定义」。
