...
### 7.3 规则副本防分叉：`scripts/formal-records.mjs`

`PORTABLE_PREDECESSORS.acceptance_package`（L50-56）现硬编码五类，是 schema 之外的第二份规则。改为按验收记录实际引用动态推导，并新增一致性断言：同一份记录在 schema、`cwf-evidence-verify`、`formal-records` 三处的可登记/前置判定不得互相矛盾。
