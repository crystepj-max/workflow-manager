...
GitHub `#106` / `#108` / `#109` / `#110` 已被 `[Accidental / Closed]` 占位 issue 占用，与本地 `FIX-109`（cnb#109）、`CHORE-110`（cnb#110）直接撞号。**GitHub issue 编号不可回收**（删除后不复用），因此处置口径需人工裁定，见 `decision-tickets/DT-01-numbering-collision.md`。
...
**1 项**：`DT-01` 历史号段冲突处置口径（A 跳号 / B 保留 CNB 号只对新任务生效 / C 一次性重编）。

该项裁定前，本票**不得**标为「本地已定义」，开工亦被阻塞。
