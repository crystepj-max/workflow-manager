...
- ~~不改命令命名空间 `/codex-review`~~ —— V1 曾列为非目标；DT-01 于 2026-09-20 裁定 B，已由本任务后续 PR 改名 `/pr-review`（见 §19 V2）。
...
- **命名空间已裁定并执行（V2）**：DT-01=B，命令改名 `/pr-review`；`parseCommand`、workflow `if:` 前缀、AGENTS.md、`.cursor/BUGBOT.md`、测试均已同步。旧命令 `/codex-review` 无兼容期，**在途 PR 里旧命令即刻失效——需在 PR/群内公告一次**。内部审计标记与文件名保留旧名（非用户可见）。
...
| 命令命名空间 | **改名 `/codex-review` → `/pr-review`（DT-01 裁定 B，已执行）** | 名字误导成本 > 在途 PR 公告成本；产品直接选定 B，无兼容期 | 产品 | 2026-09-20 |
...
- 触发通道（评论/API，DT-02）未裁，本卡按评论默认实现；命令命名空间（DT-01）已于 V2 裁定 B 并执行。
...
| V2 | 2026-09-20 | 据用户实测 Cursor 设置页修正（触发词 `@cursor review` 单一常量、Incremental/Draft/Summaries/Autofix/Effort 实况）；DT-01 裁定 B：命令改名 `/codex-review`→`/pr-review`，无兼容期 | 产品 |
