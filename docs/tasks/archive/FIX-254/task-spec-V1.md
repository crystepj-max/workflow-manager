...
| AC-06 | **CI 实机双臂对照** | 同 runner、同提交、同命令，仅切换身份来源：有身份 8 项失败 / 无身份 15 项失败 → 7 项收口链真实转绿 |

## 5. 影响面

- 蓝图与生成结果：**零影响**（未触碰 `templates/`）。
- 产品行为：**零影响**（仅 CI 工作流）。
- 对工作流程的影响：CI 引擎层失败由 15 项降为 8 项（实机双臂对照），是后续把 `validate` 设为必需状态检查的**必要但不充分**前置。

**边界声明（不得含糊）**：本票只完成「身份」一环，**`validate` 在 CI 上仍不会全绿**。剩余 8 项既存失败（`M4 机械验收脚本通过`、dev-plugin 两项 `start` 用例、`gh 不可达时降级 TMP`、`V-3`/`V-4`/`V-5`/`V-8`）需另行立票。另附带发现 `scripts/validate.mjs` 存在管道卡死风险（首次推送时 CI 曾卡 25 分钟未结束、后未复现），亦另案处理。
