...
> **版本**：v0.1.0（随本 skill 包维护）  
> **产品拍板**：自动返工上限 **3**；人工验收严格三态；定义能力升级现有「需求分析」Skill，不新建第二套入口。  
> **本文件职责**：本 skill 定义阶段所需的公共契约——Issue 九字段、用户可见状态、版本一致、「已定义」门禁、验收严格三态、返工上限。供本 skill 与后续工作流能力共用同一套语义；不依赖仓库外路径。  
> **本 skill 范围**：把任务落到「已定义」。施工 / 审查 / 验收属工作流侧后续能力。
...
Issue 正文建议使用模板：`references/issue-basics-template.md`。
...
### 6.1 与历史建设契约用词的对照（后续迁移备忘）

| 本契约（产品拍板） | 历史建设 Portable Contract 用词 | 说明 |
|---|---|---|
| `ACCEPT` | `accept` | 语义对齐 |
| `REJECT` | `reject` | 语义对齐 |
| `CONDITIONAL_PASS` | （无直接对应） | **不是**历史 `user_accepted`。历史 `user_accepted` = 知情接受**未完全满足基线**的结果；本契约禁止用该路径替代「有条件通过」。后续改造交付时以本契约三态为准。 |
...
规则摘要（供后续单任务交付实现；本 skill 不执行返工循环，但契约常量在此冻结）：
...
模板：`references/task-spec-template.md`。
...
- 定义阶段完成后状态为「已定义」；后续施工 / 审查 / 验收应从「已定义」开工，不再在交付主链内重做定义决策闭环

---

## 10. M1 明确不做

- 单任务交付完整主链（施工 / 审查 / 验收）——属工作流侧后续能力
- Execution Plan / 批量 / 定时（M3/M4）
- 有前置依赖任务的自动接续（V0.2）
- 把历史建设 handoff schema 的 `user_accepted` 立刻改名为 `CONDITIONAL_PASS`（留待后续交付改造，避免本 skill 升级扩大范围）
