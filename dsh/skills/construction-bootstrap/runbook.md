...
1. 按 `<SKILL_DIR>/assets/ai-task-define-delivery/uat-card-template.md`（源码态为 `docs/design/ai-task-define-delivery/uat-card-template.md`）生成验收卡（落盘到 run 目录，如 `uat-card.md`）。
   - **每条验收项必须声明「执行时机」**：`裁决前可观测` 或 `收口后观测`。
   - 「收口后观测」项（依赖收口/合并之后才可能存在的状态：主检出已包含本修复、本任务工作区已删除、主检出自跑全绿等）写进卡片的「收口后复核」区，**不得**混入裁决清单，**不得**作为裁决前置。
   - 一张卡至少要有**一项**「裁决前可观测」项作为裁决依据；若全部验收项都无法在裁决前观测，属需求定义不完整 → 退回定义阶段，不得进入等待验收。
   - 违反本规则的后果是**假红灯**：交付操作者照单执行时看到失败，却并非实现有缺陷（2026-09-15 LOC-023 实测）。
...
```
   ...
   🔴 **收口后复核（「执行时机：收口后观测」项的落地）**：合并完成后，由**执行体（AI）立即**执行验收卡「收口后复核」区的各项，并把**执行时间 / 命令 / 实际结果 / 结论**回填到该区与 `closeout_summary`。**不得留空、不得静默跳过**。任一项失败如实记录：属真缺陷 → **另立任务**（不在本轮静默修）；属环境或时机不成立 → 写明原因与后续触发条件，列入已知限制。本条是收口动作的内容，**不新增主链阶段或状态**。

   前置顺序提醒：`closeout_summary` 需在**删除工作区之前**写入（`cwf-record` 对 `run.work_branch` 有「实际检出必须存在」的血缘守卫；工作区已删则需临时 `git worktree add` 重建检出再写）。
