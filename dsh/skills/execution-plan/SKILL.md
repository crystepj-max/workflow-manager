---
name: execution-plan
description: "AI 任务批量调度（Execution Plan）：从候选「已定义」任务中筛选可执行项、冻结快照、按 P0、P1、P2 顺序 与定义时间排序、在最大并发内启动单任务交付并在名额释放后自动补位，最后输出批次汇总。不做需求分析/开发/审查/测试/UAT。当用户说「执行计划」「Execution Plan」「批量开工」「夜间批量」「按并发跑已定义任务」「启动批次」时使用。"
---
...
- 「本地已定义」（本地轨道）与「已定义」**开工资格相同**，必须同等纳入候选筛选；`GitHub 同步 = pending` 仅表示待补 issue，不构成开工阻塞，也不影响优先级排序。「本地准备完成，待同步」是已废止的历史状态，不再具备也不再需要开工资格。
