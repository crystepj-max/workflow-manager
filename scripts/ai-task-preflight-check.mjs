 ...
 *       [--run-baseline Vn] [--env-store <dir>]——exit 0 = 通过；exit 1 = 受阻；exit 2 = 用法错误
...
/**
 * 资格校验唯一实现（LOC-003）。读入任务基本信息与本地任务规格，返回结构化结果；
 * 不打印、不退出。CLI 薄壳只负责参数解析与输出/退出码映射。
 * @returns {Promise<{ok: boolean, failures: string[], fields: object|null}>}
 *   fields = 通过时的字段事实（含 CLI JSON 全部键 + name/defined_at 供进程内消费方使用）
 */
export async function runPreflight(issuePath, specPath, opts = {}) {
  ...
  if (failures.length) return { ok: false, failures, fields: null }
...
async function main() {
  ...
  if (args.length < 2) {
    console.error('用法: node scripts/ai-task-preflight-check.mjs <issue-basics.md> <task-spec.md> [--run-baseline Vn] [--env-store <dir>]')
    process.exit(2)
  }
  ...
  const result = await runPreflight(args[0], args[1], { runBaseline, envStore })
  if (!result.ok) {
    console.error('实施前检查未通过（执行受阻）：')
    for (const f of result.failures) console.error(`  - ${f}`)
    console.log(JSON.stringify({ ok: false, failures: result.failures, auto_rework_limit: 3 }, null, 2))
    process.exit(1)
  }
