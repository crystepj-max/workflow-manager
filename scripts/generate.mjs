...
export function compileBlueprint(bp, opts = {}) {
  ...
  lines.push(
    ...
    '  if (WS) s += \'\\n- workspace 路径：\' + WS + \'（#93 隔离工作区）\'',
    '  if (SOURCE) s += \'\\n- source 路径：\' + SOURCE + \'（Git worktree 现场）\'',
    '  if (RECORDS) s += \'\\n- records 路径：\' + RECORDS + \'（Formal Records 目录）\'',
    ...
    '}',
    'function coerceStructured(v) {',
    '  if (typeof v === \'string\') { try { const p = JSON.parse(v); if (p && typeof p === \'object\') return p } catch (e) {} }',
    '  return v',
    '}',
    ...
    '  phase(n.label || id)',
    '  return coerceStructured(await agent(prompt, opts))',
    '}',
  ...
  lines.push(
    ...
    '      return coerceStructured(await agent(prompt, itemOpts))',
    '    })',
