...
export function compileBlueprint(bp, opts = {}) {
  ...
  lines.push(
    ...
    '    if (!e) return { status: \'ENDED_NO_OUTCOME_EDGE\', stage: current, results: results, history: history }',
    '    log((n.label || current) + \' → \' + String(e.outcome))',
    '    if (e.to === HD_ID) {',
    '      return { status: \'ROUTE_HALTED\', reason: \'HUMAN_DECISION\', taskId: TASK, node: current, round: round, results: results, history: history }',
    '    }',
    '    current = e.to',
    '    continue',
    '  }',
    ...
    '  current = e.to',
    '}',
    'const done = { status: \'DONE\', taskId: TASK, round: round, results: results, history: history, completion: completionOf(lastNode) }',
