...
export function compileBlueprint(bp, opts = {}) {
  ...
  lines.push(
    ...
    '  return miss',
    '}',
    'function haltWaitingHuman(nodeId, outcome, reason, blockedEdge, overridePkg) {',
    ...
    '  const decisionId = TASK + \':\' + nodeId + \':\' + round',
    '  const ev = {',
    ...
    '  }',
    '}',
    'function choiceControlEvent(choice, blocked) {',
    '  const effects = controlOptionEffects()',
    '  return {',
    '    record_kind: ' + JSON.stringify(HD_EVENT_RECORD_KIND) + ',',
    '    trigger: ' + JSON.stringify(HD_EVENT_TRIGGER) + ',',
    ...
    '    user_choice: choice,',
    '    impact: effects[choice] || (\'选择 \' + choice),',
    '    subsequent_path: (choice === \'ADD_BUDGET\' && blocked && blocked.to) ? String(blocked.to) : choice,',
    '    created_at: new Date().toISOString(),',
    '  }',
    '}',
  ...
  lines.push(
    ...
    '  choiceEvent = choiceControlEvent(choice, A.blocked_edge || null)',
    '  if (choice === \'STOP\') return { status: \'STOPPED\', taskId: TASK, decision_id: A.decision_id, results: results, history: history, user_choice: choice, control_event: choiceEvent }',
    '  if (choice === \'USER_ACCEPTED\') return { status: \'DONE\', taskId: TASK, decision_id: A.decision_id, results: results, history: history, user_choice: choice, control_event: choiceEvent }',
    ...
    '    current = blocked.to',
    '  } else {',
    '    return { status: \'ERROR\', detail: \'业务 Decision Result 续跑未实现：\' + choice }',
    '  }',
    '}',
    'while (current !== \'$end\') {',
...
export function skillWrap(bp) {
  return [
    ...
    '   - `WAITING_HUMAN`：呈 Decision Package（why / current_state / options / subsequent_effects）；按 `decision_id` + `user_choice` 续跑。控制类 Result：`STOP` 停止本 Run、`USER_ACCEPTED` 完成且不改写原 Outcome、`ADD_BUDGET` 沿被拦边再走。',
