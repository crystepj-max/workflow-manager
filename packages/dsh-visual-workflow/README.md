...
- **模板库**：正式内置（`.generated/<id>/`，builtin=true，结构只读）+ 历史自定义种子（同目录生成物，builtin=false，可覆盖/删除，蓝图在 `templates/custom-seeds/`）+ 用户模板（`~/.dsh/visual-workflow/templates/<id>.json`）；新建 / 编辑（打开全局居中的顶层编辑工作区）/ 删除（confirm 确认）/ 刷新。
- **编辑器**（全局编辑层内）：
