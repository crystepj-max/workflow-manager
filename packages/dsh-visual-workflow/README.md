...
- **编辑器**（全局编辑层内）：
  - 画布：初始/重置时纵横居中并尽量容纳全局，空白区域支持上下左右拖动；自动分层布局（success 主链 LR，节点保持安全间距；前向跨节点边往下绕行、回退边往上绕行、无遮挡前向边直连；同节点多边按上绕/直连/下绕分层错开；起始小圆点、终点箭头、整条边同色）、点选节点/边、从节点右把手拖出连线到目标节点建边、右键画布「添加结束节点」、滚轮缩放（指针锚定）+ 缩放控件（画布内带横向/纵向滚动条）、入口徽标、$end 虚线终点、流动虚线边 + 成功/失败标签（成功蓝色、失败红色、选中主文字色加粗；when 条件悬停可见，工具栏为文档流内一行、不遮挡入口节点）。
