...
DSH 动态双半插件：在 Web 设置页提供「工作流」section——快速调整（内联画布编辑）+ 模板库 + 大抽屉精细编辑器 + 运行看板。编辑器参考 **Gold-Band 桌面端《工作流编辑器》模块**（`web/src/components/WorkflowEditor.tsx` + `workflowGraph.ts`）实现，功能、UI 结构、交互体验与其保持一致；内联快速调整整合自历史会话的 **pkg-19 双半包**（`.scratch/vwf-pkg19/`）配置页画布编辑能力。
...
激活后打开 DSH Web 设置页 → 「工作流」（内容区带纵向滚动条）：

- **快速调整**（默认 tab，整合自 pkg-19 配置页内联编辑）：模板下拉选择 + 新建/删除/刷新 + **内联画布编辑器**（拖拽连线建边、增删节点、右键添加结束节点、节点/边表单、保存校验、JSON tab），适合快速调整；需要完整配置时点 **「✎ 精细编辑」** 打开右侧 ≈1120px 大抽屉。
- **模板库**：内置「开发工作流 2.0」+ 用户模板列表；每行 快速调整 / 编辑（大抽屉）/ 删除 / 刷新。
- **编辑器**（抽屉内）：
  - 画布：自动分层布局（success 主链 LR，回退边走上方车道）、点选节点/边、从节点右把手拖出连线到目标节点建边、右键画布「添加结束节点」、滚轮缩放（指针锚定）+ 拖拽平移 + 缩放控件、入口徽标、$end 虚线终点、流动虚线边 + 成功/失败标签（when 条件悬停可见，工具栏为文档流内一行、不遮挡入口节点）。
...
- 宿主形态：pkg-19 的配置页内联画布编辑保留为「快速调整」tab（默认），另加模板库列表 + 右侧大抽屉（对应 Gold-Band 的 Sheet 抽屉）做精细配置。
