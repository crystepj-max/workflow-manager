...
- [T-06 异源 enforcement 契约](tickets/T-06-heterogeneity-contract.md) — **弱异源放行+完全同模型拒**（判定键 provider+model，消解规格措辞冲突）；save/update/validate 三处校验，engine 不拦；~~**全局强制**（凡含 dev+review 节点一律校验，缺绑定拒）~~ → **已由 LOC-021 修订为按蓝图 `heteroCheck` 档位生效**（关/弱/强，默认弱；见票内 Supersession 段与 `docs/design/blueprint-schema.md` §3.1 规则 7）；错误沿用 errors[]；AC-8 细化 6 例（T1 同模型拒/T2 弱异源过+警告/T3 真异源过/T4 缺绑定拒/T5 跳过/T6 update 同 save）；~~heteroCheck 退化为运行时日志~~ → **异源档位同时驱动校验强度与运行时日志**。blueprint-schema.md 已同步（§3.1 规则 7）。→ **T-IMP-07 ✅（2026-08-20 插件层落地）**：识别键按节点 `id` 或 `profile` 定位 dev/review，引擎规则 4/7 与 host.js 内联校验同源，测试 T7/T8 增补。→ **LOC-021 ✅（2026-09-14 档位三态化）**。
...
- [候选一：统一编译器（T-IMP-12，2026-08-20，架构评审收口）](docs/design/plugin-layer.md) — **双编译器统一为单一编译器**：`compileDsh` 改名 `compileBlueprint`（`scripts/generate.mjs`），DSH/vwf 双入口共用；宿主侧 `compileDsl` 删除，经管道取译文——内置模板读 `.generated/<id>/script.mjs`、用户模板读 `~/.dsh/skills/<id>/script.mjs`（磁盘优先、含蓝图全部增强）、临时图与编辑器实时查看走 CLI `generate.mjs compile` 兜底（DSL 逆投影回蓝图）；`vwf.compile` RPC 删除。折叠/可信度闸门/超限归因在 vwf 入口按蓝图内容生效（D3 折叠在运行层收口——编辑器图仍保留 route 节点）；对拍套件 C2 差异断言翻转为一致断言 + 逐字节一致断言（runtime-host.test.mjs H1-H6）。
- [候选一：统一投影内核（2026-09-06）](specs/vwf-projection-core/proposal.md) — `scripts/projection-core.cjs` 收敛蓝图 ↔ vwf DSL 正逆投影；生成器、Host、正式 `dist` 资产和直测共享同一字段契约。`verifyBranch` 已进入 DSL 往返，编辑器仍暂无专门 UI；Host 缺失内核时 fail-closed。
- [候选二：统一校验内核 + 业务规则前端可配置（T-IMP-13，2026-08-20，架构评审收口）](docs/design/plugin-layer.md#11-统一校验管道t-imp-13--候选二) — **三份规则集收敛为唯一内核 `scripts/validate-core.cjs`**（CJS 单文件）：结构层 `validateStructure`（走通性/定义/入口/环/条件/maxRounds ∈ [1,9] 系统上限）+ 业务规则层 `validateBlueprint`（异源硬规则/verifyBranch/output.files/requireModels 选项）；引擎 ESM import、宿主经 fs 读源码 vm 内求值缓存（热路径零子进程）；原 `validate-blueprint.mjs` 与宿主 `validateDsl`/`heteroCheck`/拓扑推导/COND_RE 删除。**契约修订（Q7/Q8）**：「增强字段不进入 DSL」→ `onMaxRounds`/`heteroCheck` 进入 DSL、编辑器控制面板可配置（异源开关/超限行为/上限 1-9 钳制），双向投影落盘；verifyBranch 节点级字段无编辑器 UI 但已进入 DSL（统一投影内核见候选一）。错误统一带坐标键 fieldKey，前端文案翻译列优化任务。验收：唯一规则集 grep 零残留、maxRounds 差集补齐、COND_RE 一致性、fieldErrors 坐标保真、Q7 闭环测试（63 引擎 + 37 host 全绿）。
- [候选四：保存闭环失败零残留（T-IMP-14，2026-08-20，架构评审收口）](docs/design/plugin-layer.md) — **skill 写盘原子化**：生成器 `user` 子命令新增 `writeUserSkill(bp, skillDir, io?)`——暂存目录 + 同父目录 rename 原子换入，任一步失败清理暂存零残留（更新场景旧版本不受影响；删除用 unlink/rmdir 规避宿主 safe-delete 钩子）；宿主 save 回滚保留为防御纵深（候选二后校验同内核同数据，回滚正常操作不可达）。原报告「两份校验形态」摩擦已由候选二消解；「三步接口」对调用方本就不存在（save RPC 单接口）。验收：66 引擎 + 37 host 全绿（含注入写盘失败的零残留/旧版本保留用例）。
- [候选五：契约一致性机器核对（C5，2026-08-20，架构评审收口）](docs/design/blueprint-schema.md) — **「人工核对」→「机器核对」**：校验内核规则 A——goal 中反引号引用的文件名必须 ⊆ 全局 `output.files` ∪ {STATE.md}（错误级，坐标 `node:<id>:goal`）；repo 级测试 T8——`dsh/roles/*.md` 反引号文件名同样 ⊆ 模板声明。精度决策：仅检查反引号引用（裸提及如 `package.json` 工程文件不检查，避免误报）；约定交付物文件名在 goal/角色文件中一律反引号（模板 7 goal + 5 角色文件已同步）。反向一致性（own files ⊆ own goal）经评估不做（Q2a：文件清单已由运行时注入，goal 不枚举不算缺陷）。验收：69 引擎 + 37 host 全绿（含规则 A 正负例 + T8）。

## Not yet specified

- **前端报错文案翻译（候选二 Q9 预留优化任务）**：后端已按坐标键（fieldKey）报错；前端把坐标键翻译为用户可理解的文案（i18n 表/映射层）——何时做、怎么做待定。
